...
Для безопасного деплоя на Render.com без хранения приватных ключей в репозитории:

1. Удалите `service_account.json` из репозитория (если он там есть):
   ```
   git rm service_account.json
   ```

2. Убедитесь, что `service_account.json` добавлен в `.gitignore` (это уже сделано)

3. Создайте переменную окружения в Render.com Dashboard:
   - Перейдите в ваш сервис на Render.com
   - Откройте раздел "Environment"
   - Добавьте переменную `GOOGLE_SERVICE_ACCOUNT_JSON`
   - Вставьте туда полное содержимое вашего `service_account.json` файла

4. Для локальной разработки вы можете продолжать использовать файл `service_account.json`

### Настройка переменных окружения

В Render.com необходимо установить следующие переменные окружения:
- `BOT_TOKEN` - Токен вашего Telegram бота
- `ADMIN_IDS` - ID администраторов (через запятую)
- `GOOGLE_SPREADSHEET_ID` - ID вашей Google таблицы
- `GOOGLE_SERVICE_ACCOUNT_JSON` - Содержимое service account JSON файла
